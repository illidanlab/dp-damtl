...
public class Client_Square_Loss_one {
	// Task node side, use least square loss in regression problem
	...
	private static double Lambda = Math.pow(10, -5); // regularization parameter, also one in.py file
	private static double eps1 = 0; // privacy parameter on q
	private static double eps2 = 0; // privacy parameter on grad
	private static double S1 = 0; // sensitivity on q_new
	private static double S2 = 0; // sensitivity on output gradient
	private static ArrayList<String> mse = new ArrayList<String>(); // array of error rate in all iterations
	private static int ITER = 10000; // Number of iterations (change)
	private static double alpha1 = 0.002; // gradient descent (change), remember to change S1 when change alpha1
	...
	public static void measurer(AMTL_Matrix data, AMTL_Matrix label, AMTL_Matrix model){
		// Mean squared error of task model in each iteration, regression problem
		...
		AMTL_Matrix x = new AMTL_Matrix(1, col, 0);
		for(int i = 0; i<col; i++){
			columns[i] = i;
		}
		...
		for(int i = 0; i<row; i++){
			...
			double tmp = 0; // temporarily store for dot product
			for(int j = 0; j < col; j = j + 1) {
				tmp = tmp + x.getDouble(j, 0)*model.getDouble(j, 0);
			}
			tmp = tmp - label.getDouble(i, 0);
			tmp = Math.pow(tmp, 2);
			e = e + tmp;
		}	
		String es = Double.toString(e/row); // MSE = \frac{1}{n}\sum_{i=1}^{n}(\hat{y}-y)^{2}
		if ((countITER % 10) == 0)
		{	
			System.out.println("Task node: " + Integer.toString(index) + ", MSE in iteration " + Integer.toString(countITER) + " is " + es);
		}
		mse.add(es); 
	}
	...
	public static void main(String[] args) {
		...
		*/
		...
		/**************************************************Data read start**************************************************/
		// objective is least square loss with l2 regularization. See operators. (change) from this line
		...
		*/
		// school.mat
		try{ 
			X_load = MatrixIO.loadCSV("schoolCSVdata"+index);
		}catch (IOException e1){
			throw new RuntimeException(e1);
		}
		...
		// Alzheimer disease
		try{ 
			X_load = MatrixIO.loadCSV("AlzheimerCSVdatar"+index);
		}catch (IOException e1){
			throw new RuntimeException(e1);
		}
		...
		*/
		...
		// Synthetic
		try{ // label vector y
			y_load = MatrixIO.loadCSV("label"+index);
		...
		*/
		// school.mat
		try{ 
			y_load = MatrixIO.loadCSV("schoolCSVscore"+index);
		}catch (IOException e2){
			throw new RuntimeException(e2);
		}
		/*
		// Alzheimer disease 
		try{ // label vector y
			y_load = MatrixIO.loadCSV("AlzheimerCSVscore"+index);
		}catch (IOException e2){
			throw new RuntimeException(e2);
		}
		*/
		...
		AMTL_Matrix q_old = new AMTL_Matrix(q0,Blas); // Old q, computed and left in this task node in previous iteration, note the fact that it is equal to q_ini in the first iteration
		...
		try{
			...
			for(int j = 0; j < ITER; j++){
				...
				if(clientMsg.getError() == 0){
					...
					if(j == 0){
						q_new = forward1.SquareLossReg_Forward(X_train, y_train, p_new, q_ini, Lambda); // First iteration q is initialized by the minimizer of L(q) with p=0
					}else{
						q_new = forward1.SquareLossReg_Forward(X_train, y_train, p_new, q_old, Lambda); // Other iteration q is the previous one
					}
					...
					Operators forward2 = new Operators(alpha1); // StepSize alpha1 here is useless 					
					grad = forward2.SquareLossGrad_Forward(X_train, y_train, p_new, q_new); // p here should be p_new and q should be q_new
					...
					dpn1.compute();
					//AMTL_Matrix Dpn1 = new AMTL_Matrix(dim, 1, Blas); // A test of set noise vector on q to 0
			...
			DenseMatrix64F a = new DenseMatrix64F(1, mse.size()); // in matrix form
			for(int i=0; i<mse.size(); i++){
	            a.set(0, i,  Double.parseDouble(mse.get(i)));
			} 
			try{ // write at the end of program
				MatrixIO.saveCSV(a,"MSE"+Integer.toString(index)); // the file is stored under /Javaws/DAMTLDP
			}catch (IOException e1){
				throw new RuntimeException(e1);
			}
